...
if __name__ == '__main__':
    ...
    myExps   = [0.8]
    myCoefs  = [0.8]
    b = BasisFunction(origin=myOrigin,shell=myShell,exps=myExps,coefs=myCoefs)

    myOrigin = [0.0, 0.0, 0.0]
    myShell  = (0,0,0) # p-orbitals would be (1,0,0) or (0,1,0) or (0,0,1), etc.
    myExps   = [0.3]
    myCoefs  = [0.8]
    c = BasisFunction(origin=myOrigin,shell=myShell,exps=myExps,coefs=myCoefs)
    ...
    myExps   = [1.66]
    myCoefs  = [0.8]
    d = BasisFunction(origin=myOrigin,shell=myShell,exps=myExps,coefs=myCoefs)


    print("S: ",S(a, a), S(a, b), "\n", S(b, a), S(b, b))
    print("T: ",T(a, a), T(a, b), "\n", T(b, a), T(b, b))
    print("V: ",2*V(a, a, [0.0, 0.0, 0.0]), 2*V(a, b, [0.0, 0.0, 0.0]), "\n", 2*V(b, a, [0.0, 0.0, 0.0]), 2*V(b, b, [0.0, 0.0, 0.0]))

    from Utilities.Wavefunction import GaussianBasisS as gbfs

    print("S: ", gbfs.Spq(0, 0, [0.2], [0.2]), gbfs.Spq(0, 0, [0.2], [0.8]), "\n", gbfs.Spq(0, 0, [0.8], [0.2]), gbfs.Spq(0, 0, [0.8], [0.8]))
    print("T: ", gbfs.Tpq(0, 0, [0.2], [0.2]), gbfs.Tpq(0, 0, [0.2], [0.8]), "\n", gbfs.Tpq(0, 0, [0.8], [0.2]), gbfs.Tpq(0, 0, [0.8], [0.8]))
    print("V: ", gbfs.Vcpq(0, 0, 0, [0.2], [0.2], 2), gbfs.Vcpq(0, 0, 0, [0.2], [0.8], 2), "\n", gbfs.Vcpq(0, 0, 0, [0.8], [0.2], 2), gbfs.Vcpq(0, 0,  0, [0.8], [0.8], 2))

    pos = np.array([0.0, 0.0, 0.0])
    s = (0,0,0)

    print(electron_repulsion(0.2,s,pos,
                             0.8,s,pos,
                             0.3,s,pos,
                             1.66,s,pos))


    print(gbfs.pqrs(pos, pos, pos, pos, [0.2], [0.8], [0.3], [1.66]))